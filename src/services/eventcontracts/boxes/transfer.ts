...
import { buyBoxEvent } from "./buyBox"
    ...
    try {
        ...
        await session.withTransaction(async () => {
            if (from === null_address && to !== null_address) {
                ...
                if (!buyBoxEvent[`${event.transactionHash}_${to}`]){
                    buyBoxEvent[`${event.transactionHash}_${to}`] = []
                } 
                buyBoxEvent[`${event.transactionHash}_${to}`].push(newBox)
